...
async def orders(status: str = Query(default="open", description="open|closed|all")) -> Any:
    """Zlecenia (domyślnie otwarte)."""
    return _guard(lambda: client.get_orders(status))


@router.post("/orders", response_model=Order)
async def submit_order(req: OrderRequest) -> Any:
...
async def cancel(order_id: str) -> Any:
...
async def close(symbol: str) -> Any:
